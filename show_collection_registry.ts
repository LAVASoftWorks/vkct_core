/**
 * Collection registry lister
 * Author: lava.caballero@gmail.com
 *
 * Usage: ts-node show_collection_registry.ts
 */
...
(async () => {
    ...
    const registry = await program.account.collectionRegistry.fetch(collectionRegistryPda);

    console.log("📜 Collection Registry");
    console.log("Admin:", registry.admin.toBase58());
    console.log("Whitelisted Collections:");
    registry.collections.forEach((mint: PublicKey, i: number) => {
